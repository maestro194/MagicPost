...
export const signup = async (req, res, next) => {
    ...
    const {username, email, password, fullname, type, officeCode} = req.body;
    const id = 1 + Math.floor(Math.random() * 1000000);
    const hashedPassword = bcryptjs.hashSync(password, 10);
    const newUser = new User({id, username, password: hashedPassword, email, fullname, type, officeCode});
